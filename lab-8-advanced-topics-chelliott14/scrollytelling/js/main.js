...
d3.csv('data/assignments.csv').then(_data => {
  ...
  d3.select('#easy-homeworks-count').text(data.filter(d => d.difficulty == 'Easy').length);
  d3.select('#difficult-homeworks-count').text(data.filter(d => d.difficulty == 'Difficult').length);

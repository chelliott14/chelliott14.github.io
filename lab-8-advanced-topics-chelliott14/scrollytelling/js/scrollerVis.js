class ScrollerVis {
  ...
  initVis() {
    ...
    // Identify assignments with the longest completion time
  ...
  step1() {
    ...
    vis.rect.transition()
        .attr('fill', d => d.difficulty=='Easy' ? 'red' : '#ccc');
  }
  ...
  step2() {
    ...
    vis.rect.transition()
        .attr('fill', d => d.difficulty=='Difficult' ? 'red' : '#ccc');
  }
  ...
  step3() {
    ...
    vis.rect.transition()
        .attr('opacity', 1)
        .attr('fill', d => d.name==vis.dataLongestAssignment.name ? 'red' : '#ccc')
        .attr('width', d => vis.config.cellWidth)
        .attr('height', d => vis.config.cellHeight)
        .attr('x', (d, i) => i % vis.config.columns * (vis.config.cellWidth + vis.config.cellSpacing))
        .attr('y', (d, i) => Math.floor(i / vis.config.columns) % vis.config.rows * (vis.config.cellHeight + vis.config.cellSpacing));

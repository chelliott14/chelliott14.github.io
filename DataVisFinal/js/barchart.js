class Barchart {
  ...
  updateVis() {
    ...
    vis.xValue = d => d.name;
    //vis.yValue = d => d.early_career_pay - d.out_of_state_total;
    vis.colorValue = d => d.enrollment_bin;

    vis.data.sort((a, b) => vis.yValue(b) - vis.yValue(a));
    vis.fullData = vis.data;

    //vis.barWidth = Math.min(d3.max(vis.data.map(d => d.name.length)) * 4, 50);
    vis.numBars = Math.min(10, vis.data.length);
    ...
    vis.chart.append('line')
      ...
      .attr('stroke', '#000')
      .attr('stroke-width', 1)
      .attr('stroke-dasharray', '3,3');  // Optional: dashed line
